...
class SimpleCNN_CIFAR100(nn.Module):
    def __init__(self, num_classes=100):
        ...
        # Reduce the complexity of the fully connected layer
        self.fc1 = nn.Linear(32 * 16 * 16, 256)
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, x):
        intermediate_outputs = []  # To collect outputs for visualization or other purposes

        x = F.relu(self.conv1(x))
        x = self.pool(x)
        intermediate_outputs.append(x)  # Store intermediate output after pooling

        x = x.view(-1, 32 * 16 * 16)  # Flatten the output for the fully connected layer
        latent_features = F.relu(self.fc1(x))  # This will be considered as the latent features
        intermediate_outputs.append(latent_features)  # Store latent features

        x = self.fc2(latent_features)
        return x, latent_features, intermediate_outputs  # Return all required outputs
...
def display_visualization():
    try:
        ...
        #    save_report(epoch, avg_loss, accuracy, alpha_lr_value, beta_lr_value, gamma_lr_value, selected_layer, fig)
